<template>
  <a-modal v-model="show" title="修改题库" @cancel="onClose" :width="800">
    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="20">
        ...
        <a-col :span="12">
          <a-form-item label='题库类型' v-bind="formItemLayout">
            <a-select v-decorator="[
              'type',
              { rules: [{ required: true, message: '请输入题库类型!' }] }
              ]">
              <a-select-option value="1">通知</a-select-option>
              <a-select-option value="2">题库</a-select-option>
            </a-select>
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label='题库状态' v-bind="formItemLayout">
            <a-select v-decorator="[
              'rackUp',
              { rules: [{ required: true, message: '请输入题库状态!' }] }
              ]">
        ...
        <a-col :span="24">
          <a-form-item label='题库内容' v-bind="formItemLayout">
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      this.form.validateFields((err, values) => {
        ...
        if (!err) {
          this.loading = true
          this.$put('/business/question-bank', {
            ...values
          }).then((r) => {
            this.reset()
            this.$emit('success')
