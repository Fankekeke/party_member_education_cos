...
export default {
  ...
  methods: {
    queryContacts () {
      this.$get(`/business/post-info/user/detail`, {
    ...
    collectUserCheck (deleteFlag) {
      this.$post(`/business/focus-info`, {userId: this.currentUser.userId, collectUserId: this.postDetail.userId, deleteFlag}).then((r) => {
        this.postReplyDetail(this.postDetail)
        this.$message.success(deleteFlag === 0 ? '关注成功！' : '取消关注成功！')
      })
    },
    collectPostCheck (deleteFlag) {
      this.$post(`/business/collect-info`, {userId: this.currentUser.userId, postId: this.postDetail.id, deleteFlag}).then((r) => {
        this.postReplyDetail(this.postDetail)
        this.$message.success(deleteFlag === 0 ? '收藏贴子成功！' : '取消收藏成功！')
      })
    },
    collectByUser (postId) {
      this.$get(`/business/post-info/collcet`, {userId: this.currentUser.userId, postId}).then((r) => {
    ...
    getPostList (tagId) {
      this.loading = true
      this.$get(`/business/post-info/tagByUser`, {
        tagId,
        userId: this.currentUser.userId
      }).then((r) => {
    ...
    onSearch (key) {
      if (key !== '') {
        ...
        this.tabKey = 9999
        this.tabName = '搜索'
        this.$get(`/business/post-info/listByUser`, {
          key,
          userId: this.currentUser.userId
        }).then((r) => {
          this.postList = r.data.data
